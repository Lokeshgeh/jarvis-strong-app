  ...
  return (
    <div className="content-safe px-4">
      <div className="mb-4 flex items-center justify-between gap-4">
        <button type="button" onClick={onBack} className="rounded-full border border-black/5 bg-white px-4 py-2 text-sm text-text2 shadow-[0_10px_22px_rgba(110,94,74,0.08)]">
          Back
        </button>
      ...
      <div className="rounded-[24px] border border-[#efcfbf] bg-gradient-to-br from-[#fff4ed] to-card p-5 shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
        <div className="grid gap-5 sm:grid-cols-[1.1fr_0.9fr]">
          <div>
            ...
            <label className="mt-4 block rounded-2xl border border-black/5 bg-white p-3">
            ...
            <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
              <button type="button" onClick={() => onStart({ ...routine, name: draftName, routine_exercises: draftExercises })} className="rounded-full bg-orange px-5 py-3 font-bold text-white">
                Start Routine
              </button>
              <button type="button" onClick={() => onSave({ id: routine.id, name: draftName, exercises: draftExercises })} className="rounded-2xl border border-black/5 bg-white px-4 py-3 text-sm text-text2">
                Save Edit
              </button>
              <button
                type="button"
                ...
                className="rounded-2xl border border-black/5 bg-white px-4 py-3 text-sm text-text2"
              >
                Share Routine
              </button>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {["Front", "Back"].map((label) => (
              <div key={label} className="rounded-[20px] border border-black/5 bg-[#faf7f3] p-4 text-center">
                <div className="mx-auto flex h-40 w-24 items-center justify-center rounded-[999px] border border-[#efcfbf] bg-[radial-gradient(circle_at_center,rgba(227,106,67,0.14),transparent_70%)]">
                  <div className="h-28 w-12 rounded-[999px] border border-[#efcfbf] bg-[#f8d4c6]" />
                </div>
      ...
      <div className="mt-5 space-y-4">
        {draftExercises.map((exercise, index) => (
          <section key={exercise.id ?? `${exercise.exercise_name}-${index}`} className="rounded-[24px] border border-black/5 bg-card p-5 shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
            <div className="mb-4 flex items-center justify-between gap-3">
              ...
              <span className="rounded-full bg-[#fff4ed] px-3 py-1 text-xs font-semibold text-orange">{exercise.sets} planned sets</span>
            </div>
            ...
            <div className="mt-3 space-y-2">
              {Array.from({ length: Number(exercise.sets) }).map((_, setIndex) => (
                <div key={setIndex + 1} className="grid grid-cols-4 gap-2 rounded-2xl border border-black/5 bg-[#faf7f3] p-3 text-sm text-text">
                  ...
                  <input
                    ...
                    className="rounded-xl border border-black/5 bg-white px-3 py-2 text-text outline-none"
                  />
                  <input
                    type="number"
                    value={exercise.reps}
