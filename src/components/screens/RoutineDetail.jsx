  ...
  return (
    <div className="content-safe px-4">
      <div className="mb-4 flex items-center justify-between gap-4">
        <button type="button" onClick={onBack} className="rounded-full border border-white/10 px-4 py-2 text-sm text-text2">? Back</button>
        ...
        <div className="grid gap-5 sm:grid-cols-[1.1fr_0.9fr]">
          <div>
            ...
            <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
              <button type="button" onClick={() => onStart({ ...routine, name: draftName, routine_exercises: draftExercises })} className="rounded-full bg-blue px-5 py-3 font-bold text-[#03131d]">Start Routine ?</button>
              <button type="button" onClick={() => onSave({ id: routine.id, name: draftName, exercises: draftExercises })} className="rounded-2xl border border-white/10 px-4 py-3 text-sm text-text2">Save Edit ?</button>
              <button type="button" onClick={async () => { await navigator.clipboard.writeText(`${window.location.origin}?routine=${routine.id}`); window.alert("Routine link copied."); }} className="rounded-2xl border border-white/10 px-4 py-3 text-sm text-text2">Share Routine ??</button>
            </div>
          </div>
      ...
      <div className="mt-5 space-y-4">
        {draftExercises.map((exercise, index) => (
          <section key={exercise.id ?? `${exercise.exercise_name}-${index}`} className="rounded-[24px] border border-white/10 bg-card p-5">
            ...
            <div className="grid grid-cols-4 gap-2 text-xs uppercase tracking-[0.24em] text-text3"><div>Set</div><div>Prev</div><div>KG</div><div>Reps</div></div>
            <div className="mt-3 space-y-2">{Array.from({ length: Number(exercise.sets) }).map((_, setIndex) => (<div key={setIndex + 1} className="grid grid-cols-4 gap-2 rounded-2xl border border-white/8 bg-[#090912] p-3 text-sm text-text"><div className="flex items-center">{setIndex + 1}</div><div className="flex items-center text-text3">-</div><input type="number" value={exercise.kg} onChange={(event) => setDraftExercises((current) => current.map((entry, rowIndex) => (rowIndex === index ? { ...entry, kg: Number(event.target.value) } : entry)))} className="rounded-xl bg-white/5 px-3 py-2 text-text outline-none" /><input type="number" value={exercise.reps} onChange={(event) => setDraftExercises((current) => current.map((entry, rowIndex) => (rowIndex === index ? { ...entry, reps: Number(event.target.value) } : entry)))} className="rounded-xl bg-white/5 px-3 py-2 text-text outline-none" /></div>))}</div>
          </section>
        ))}
      </div>
    </div>
