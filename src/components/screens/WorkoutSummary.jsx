  ...
  return (
    <div className="content-safe px-4">
      <div className="relative overflow-hidden rounded-[28px] border border-gold/20 bg-gradient-to-br from-[#171126] to-card p-6">
        <div className="confetti-layer" aria-hidden>
          {Array.from({ length: 18 }).map((_, index) => (
            <span key={index} style={{ left: `${5 + index * 5}%`, backgroundColor: colors[index % colors.length], animationDelay: `${index * 0.14}s` }} />
          ))}
        </div>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        {[["Duration", `${Math.round(summary.duration_seconds / 60)} min`], ["Volume", `${Math.round(summary.volume_kg)} kg`], ["Sets", `${summary.sets_completed}`], ["Records", `${summary.records_broken}`]].map(([label, value]) => (
          <div key={label} className="rounded-[24px] border border-white/10 bg-card p-5">
            <p className="text-xs uppercase tracking-[0.24em] text-text3">{label}</p>
            <p className="mt-2 text-3xl font-bold text-text">{value}</p>
          </div>
        ))}
      </div>
      <div className="mt-5 rounded-[24px] border border-blue/20 bg-card p-5">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.24em] text-text3">Muscles trained</p>
            <h3 className="mt-2 text-xl font-bold text-text">{summary.muscles.join(" - ")}</h3>
          </div>
          <button
            type="button"
            onClick={async () => {
              await navigator.clipboard.writeText(`${summary.routine_name} - ${summary.sets_completed} sets - ${summary.xpGained} XP`);
            }}
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          {["Front", "Back"].map((label) => (
            <div key={label} className="rounded-[20px] border border-white/8 bg-[#090912] p-4 text-center">
              <div className="mx-auto h-36 w-24 rounded-[999px] border border-blue/25 bg-[radial-gradient(circle_at_center,rgba(0,191,255,0.2),transparent_70%)]" />
