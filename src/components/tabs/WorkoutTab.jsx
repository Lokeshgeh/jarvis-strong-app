        ...
        <div className="space-y-5">
          <section className="rounded-[26px] border border-green/20 bg-[#0D2B1A] p-5 shadow-glow">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button type="button" onClick={() => onStartWorkout(todayRoutine)} className="rounded-full bg-blue px-4 py-3 text-sm font-bold text-[#03131d]">
                Start
              </button>
            </div>
              ...
              <div className="rounded-[24px] border border-white/10 bg-[#090912] p-5">
                <div className="grid gap-4 sm:grid-cols-3">
                  ...
                  <label className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    ...
                    <select value={focus} onChange={(event) => setFocus(event.target.value)} className="w-full rounded-xl bg-[#090912] px-3 py-3 text-text outline-none">
                  ...
                  <label className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    ...
                    <input type="number" min="30" max="120" value={duration} onChange={(event) => setDuration(Number(event.target.value))} className="w-full rounded-xl bg-[#090912] px-3 py-3 text-text outline-none" />
                  </label>
                </div>
                <div className="mt-4 rounded-[24px] border border-blue/20 bg-blue/10 p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" onClick={() => onStartWorkout(generatedRoutine)} className="rounded-full bg-blue px-4 py-3 text-sm font-bold text-[#03131d]">
                      Launch Workout
                    </button>
                  </div>
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => onOpenRoutineModal()} className="rounded-full bg-blue px-4 py-2 text-sm font-semibold text-[#03131d]">
                + Add Routine
              </button>
            </div>
            <div className="space-y-3">
              {routines.map((routine) => (
                <article key={routine.id} className="rounded-[24px] border border-white/10 bg-card p-5">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                      <button type="button" onClick={() => onOpenRoutine(routine)} className="rounded-full border border-blue/30 px-3 py-2 text-sm text-blue">Open</button>
                      <button type="button" onClick={() => onOpenRoutineModal(routine)} className="rounded-full border border-white/10 px-3 py-2 text-sm text-text2">Edit</button>
                      <button type="button" onClick={() => onDuplicateRoutine(routine)} className="rounded-full border border-white/10 px-3 py-2 text-sm text-text2">Duplicate</button>
                      <button type="button" onClick={() => onDeleteRoutine(routine.id)} className="rounded-full border border-red/30 px-3 py-2 text-sm text-red">Delete</button>
                    </div>
                  </div>
                  <div className="mt-4 grid gap-2">
                    {routine.routine_exercises.map((exercise) => (
                      <div key={`${routine.id}-${exercise.id}`} className="flex items-center justify-between rounded-2xl border border-white/8 bg-[#090912] px-4 py-3">
                        <div>
      ...
      {activeTab === "myPlan" && (
        <div className="space-y-5">
          <div className="rounded-[28px] border border-blue/20 bg-gradient-to-br from-blue/15 to-card p-6 text-center">
            <div className="mx-auto flex h-28 w-28 items-center justify-center rounded-full border border-blue/20 bg-[#090912] text-blue">
          ...
          <div className="rounded-[24px] border border-white/10 bg-card p-5">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button type="button" onClick={() => onOpenRoutineModal()} className="rounded-full border border-white/10 px-4 py-2 text-sm text-text2">
                + Assign Workout
              </button>
            </div>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {weeklyPlan.map((entry) => (
                <div key={entry.day} className={`rounded-[22px] border p-4 ${entry.routine === "Rest" ? "border-white/10 bg-[#090912]" : "border-blue/20 bg-blue/10"}`}>
                  <p className="text-xs uppercase tracking-[0.28em] text-text3">{entry.day}</p>
          ...
          <div className="rounded-[24px] border border-white/10 bg-card p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-5">
              {[["Active hours", scheduleMeta.activeHours], ["Pure work", scheduleMeta.pureWork], ["Gym", scheduleMeta.gym], ["Recovery", scheduleMeta.recovery], ["Sleep", scheduleMeta.sleepWindow]].map(([label, value]) => (
                <div key={label} className="rounded-2xl border border-white/10 bg-[#090912] p-4 text-center">
            ...
            {scheduleBlocks.map((block) => {
              ...
              return (
                <button
                  ...
                  className={`w-full rounded-[24px] border p-5 text-left transition ${complete ? "border-green/30 bg-green/10" : "border-white/10 bg-card hover:border-blue/30"}`}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="rounded-full bg-white/5 px-3 py-1 text-xs uppercase tracking-[0.24em] text-text3">{block.time}</span>
                        <span className="rounded-full bg-blue/10 px-3 py-1 text-xs font-semibold text-blue">{block.tag}</span>
                      </div>
                    ...
                    <div className={`flex h-10 w-10 items-center justify-center rounded-full border ${complete ? "border-green/30 bg-green text-[#07140d]" : "border-white/10 bg-[#090912] text-text3"}`}>
          ...
          <div className="rounded-[24px] border border-white/10 bg-card p-5">
            ...
            <ul className="mt-4 space-y-2 text-sm text-text2">
              {scheduleMeta.laws.map((law) => (
                <li key={law} className="rounded-2xl border border-white/8 bg-[#090912] px-4 py-3">
                  {law}
                </li>
              ))}
            </ul>
