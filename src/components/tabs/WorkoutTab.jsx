...
function SegmentTabs({ items, active, onChange }) {
  return (
    <div className="no-scrollbar flex gap-2 overflow-x-auto rounded-full bg-[#e6e1da] p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.85)]">
      {items.map((item) => (
        <button
          key={item.id}
          type="button"
          onClick={() => onChange(item.id)}
          className={`rounded-full px-4 py-2 text-sm font-semibold transition ${active === item.id ? "bg-white text-text shadow-[0_10px_18px_rgba(110,94,74,0.1)]" : "text-text2"}`}
        >
          {item.label}
        </button>
      ))}
...
function BodyDiagramCard() {
  return (
    <div className="rounded-[24px] border border-black/5 bg-white p-5 shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
      <div className="grid grid-cols-2 gap-4">
        {["Front", "Back"].map((label) => (
          <div key={label} className="rounded-[20px] border border-black/5 bg-[#faf7f3] p-4 text-center">
        ...
        <div className="space-y-5">
          <section className="rounded-[26px] border border-[#efcfbf] bg-[linear-gradient(135deg,#f7b39a_0%,#e36a43_100%)] p-5 text-white shadow-[0_18px_34px_rgba(227,106,67,0.22)]">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button type="button" onClick={() => onStartWorkout(todayRoutine)} className="rounded-full bg-white px-4 py-3 text-sm font-bold text-orange">
                Start
              </button>
            </div>
          ...
          <section className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <button type="button" onClick={() => onStartWorkout({ name: "Empty Workout", routine_exercises: [] })} className="rounded-[24px] border border-black/5 bg-card p-5 text-left transition hover:border-[#efcfbf] hover:bg-card-hover">
              ...
              <button type="button" onClick={() => setGeneratorOpen((current) => !current)} className="rounded-[24px] border border-black/5 bg-card p-5 text-left transition hover:border-[#efcfbf] hover:bg-card-hover">
            ...
            {generatorOpen && (
              <div className="rounded-[24px] border border-black/5 bg-card p-5 shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
                <div className="grid gap-4 sm:grid-cols-3">
                  <label className="rounded-2xl border border-black/5 bg-[#faf7f3] p-4">
                  ...
                  <label className="rounded-2xl border border-black/5 bg-[#faf7f3] p-4">
                    ...
                    <select value={focus} onChange={(event) => setFocus(event.target.value)} className="w-full rounded-xl border border-black/5 bg-white px-3 py-3 text-text outline-none">
                  ...
                  <label className="rounded-2xl border border-black/5 bg-[#faf7f3] p-4">
                    ...
                    <input type="number" min="30" max="120" value={duration} onChange={(event) => setDuration(Number(event.target.value))} className="w-full rounded-xl border border-black/5 bg-white px-3 py-3 text-text outline-none" />
                  </label>
                </div>
                <div className="mt-4 rounded-[24px] border border-[#efcfbf] bg-[#fff4ed] p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs uppercase tracking-[0.24em] text-orange">Generated</p>
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => onOpenRoutineModal()} className="rounded-full bg-[#1f1e1c] px-4 py-2 text-sm font-semibold text-white">
                + Add Routine
              </button>
            </div>
            <div className="space-y-3">
              {routines.map((routine) => (
                <article key={routine.id} className="rounded-[24px] border border-black/5 bg-card p-5 shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                      <button type="button" onClick={() => onOpenRoutine(routine)} className="rounded-full border border-black/5 bg-[#faf7f3] px-3 py-2 text-sm text-text">Open</button>
                      <button type="button" onClick={() => onOpenRoutineModal(routine)} className="rounded-full border border-black/5 bg-transparent px-3 py-2 text-sm text-text2">Edit</button>
                      <button type="button" onClick={() => onDuplicateRoutine(routine)} className="rounded-full border border-black/5 bg-transparent px-3 py-2 text-sm text-text2">Duplicate</button>
                      <button type="button" onClick={() => onDeleteRoutine(routine.id)} className="rounded-full border border-[#efcfbf] bg-[#fff5ef] px-3 py-2 text-sm text-red">Delete</button>
                    </div>
                  </div>
                  <div className="mt-4 grid gap-2">
                    {routine.routine_exercises.map((exercise) => (
                      <div key={`${routine.id}-${exercise.id}`} className="flex items-center justify-between rounded-2xl border border-black/5 bg-[#faf7f3] px-4 py-3">
                        <div>
                          <p className="font-medium text-text">{exercise.exercise_name}</p>
                          <p className="text-xs text-text3">{exercise.muscle_group}</p>
                        </div>
                        <p className="font-mono text-sm text-orange">{exercise.sets} x {exercise.reps}</p>
                      </div>
                    ))}
                  </div>
                </article>
      ...
      {activeTab === "myPlan" && (
        <div className="space-y-5">
          <div className="rounded-[28px] border border-[#efcfbf] bg-gradient-to-br from-[#fff4ed] to-card p-6 text-center shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
            <div className="mx-auto flex h-28 w-28 items-center justify-center rounded-full border border-[#efcfbf] bg-white text-orange">
          ...
          {planBuilderOpen && (
            <div className="rounded-[24px] border border-black/5 bg-card p-5 shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
              <div className="mb-4 flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.24em] text-orange">Exercise planner</p>
                ...
                <button type="button" onClick={() => setSubTab("workout", "schedule")} className="rounded-full border border-black/5 bg-[#faf7f3] px-4 py-2 text-sm text-text2">
                  View Schedule
                </button>
              </div>
              <div className="space-y-4">
                {weeklyExercisePlan.map((entry) => (
                  <article
                    key={entry.id}
                    className={`rounded-[24px] border p-5 ${entry.isRecoveryDay ? "border-black/5 bg-[#faf7f3]" : "border-[#efcfbf] bg-[#fff4ed]"}`}
                  >
                      ...
                      {!entry.isRecoveryDay && (
                        <div className="flex flex-wrap gap-2">
                          ...
                          <button
                            ...
                            className="rounded-full border border-black/5 bg-white px-4 py-2 text-sm text-text2"
                          >
                            Open Plan
                          </button>
                        </div>
                    ...
                    {entry.isRecoveryDay ? (
                      <div className="mt-4 rounded-2xl border border-black/5 bg-white px-4 py-3 text-sm text-text2">
                        Recovery focus day. Keep mobility, walking, and light activation work here.
                      </div>
                    ) : (
                      <div className="mt-4 grid gap-3">
                        {entry.exercises.map((exercise) => (
                          <div key={`${entry.id}-${exercise.exercise_name}`} className="flex items-center justify-between rounded-2xl border border-black/5 bg-white px-4 py-3">
                            <div>
                              <p className="font-semibold text-text">{exercise.exercise_name}</p>
                              <p className="text-xs text-text3">{exercise.muscle_group}</p>
                            </div>
                            <p className="text-sm font-mono text-orange">
          ...
          <div className="rounded-[24px] border border-black/5 bg-card p-5 shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button type="button" onClick={() => onOpenRoutineModal()} className="rounded-full border border-black/5 bg-[#faf7f3] px-4 py-2 text-sm text-text2">
                + Assign Workout
              </button>
            </div>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {weeklyPlan.map((entry) => (
                <div key={entry.day} className={`rounded-[22px] border p-4 ${entry.routine === "Rest" ? "border-black/5 bg-[#faf7f3]" : "border-[#efcfbf] bg-[#fff4ed]"}`}>
                  <p className="text-xs uppercase tracking-[0.28em] text-text3">{entry.day}</p>
          ...
          <div className="rounded-[24px] border border-black/5 bg-card p-5 shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-5">
              {[["Active hours", scheduleMeta.activeHours], ["Pure work", scheduleMeta.pureWork], ["Gym", scheduleMeta.gym], ["Recovery", scheduleMeta.recovery], ["Sleep", scheduleMeta.sleepWindow]].map(([label, value]) => (
                <div key={label} className="rounded-2xl border border-black/5 bg-[#faf7f3] p-4 text-center">
                  <p className="text-xs uppercase tracking-[0.24em] text-text3">{label}</p>
                  <p className="mt-2 text-xl font-bold text-orange">{value}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            {scheduleBlocks.map((block) => {
              ...
              return (
                <button
                  ...
                  className={`w-full rounded-[24px] border p-5 text-left transition ${complete ? "border-green/30 bg-green/10" : "border-black/5 bg-card hover:border-[#efcfbf]"}`}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="rounded-full bg-[#f1ece6] px-3 py-1 text-xs uppercase tracking-[0.24em] text-text3">{block.time}</span>
                        <span className="rounded-full bg-[#fff4ed] px-3 py-1 text-xs font-semibold text-orange">{block.tag}</span>
                      </div>
                    ...
                    <div className={`flex h-10 w-10 items-center justify-center rounded-full border ${complete ? "border-green/30 bg-green text-[#07140d]" : "border-black/5 bg-[#faf7f3] text-text3"}`}>
          ...
          <div className="rounded-[24px] border border-black/5 bg-card p-5 shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
            ...
            <ul className="mt-4 space-y-2 text-sm text-text2">
              {scheduleMeta.laws.map((law) => (
                <li key={law} className="rounded-2xl border border-black/5 bg-[#faf7f3] px-4 py-3">
                  {law}
                </li>
              ))}
            </ul>
