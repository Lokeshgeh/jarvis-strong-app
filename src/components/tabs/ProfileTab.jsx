...
export default function ProfileTab({ profile, workouts, achievements, routines, goals, onSaveProfile, onOpenSettings, onOpenRoutine, onOpenInfo }) {
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-[28px] border border-white/10 bg-card p-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <button type="button" onClick={onOpenSettings} className="rounded-full border border-white/10 p-3 text-text2">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          {achievements.map((achievement) => (
            <button
              ...
              className={`rounded-[22px] border p-4 text-left ${achievement.unlocked ? "border-gold/20 bg-gold/10" : "border-white/10 bg-[#090912]"}`}
            >
      ...
      <section className="rounded-[24px] border border-white/10 bg-card p-5">
        ...
        <div className="mt-4 grid grid-cols-3 gap-3">
          {[["Height", "170 cm"], ["Weight", "49.8 kg"], ["BMI", "17.2"]].map(([label, value]) => (
            <div key={label} className="rounded-2xl border border-white/8 bg-[#090912] p-4">
              <p className="text-xs uppercase tracking-[0.24em] text-text3">{label}</p>
      ...
      <section className="rounded-[24px] border border-white/10 bg-card p-5">
        ...
        <div className="mt-4 space-y-3">
          {routines.map((routine) => (
            <button key={routine.id} type="button" onClick={() => onOpenRoutine(routine)} className="flex w-full items-center justify-between rounded-2xl border border-white/8 bg-[#090912] p-4 text-left">
              <div>
      ...
      <section className="rounded-[24px] border border-purple/20 bg-gradient-to-br from-purple/15 to-card p-5">
