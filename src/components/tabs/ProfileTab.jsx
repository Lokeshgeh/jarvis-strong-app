...
export default function ProfileTab({ profile, workouts, achievements, routines, goals, onSaveProfile, onOpenSettings, onOpenRoutine, onOpenInfo }) {
  const stats = {
    ...
    streak: profile?.streak ?? 46,
    records: workouts.reduce((sum, workout) => sum + Number(workout.records_broken ?? 0), 0),
  };

  return (
    <div className="space-y-5">
      <section className="rounded-[28px] border border-black/5 bg-card p-6 shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
        <div className="flex items-start justify-between gap-4">
          ...
          <button type="button" onClick={onOpenSettings} className="rounded-full border border-black/5 bg-[#faf7f3] p-3 text-text2">
        ...
        <div className="mt-5">
          <div className="flex items-center justify-between text-xs uppercase tracking-[0.24em] text-text3">
            ...
            <span>{profile?.xp ?? 410} XP</span>
          </div>
          <div className="mt-2 h-3 overflow-hidden rounded-full bg-[#f0ebe5]">
            <div className="h-full rounded-full bg-blue" style={{ width: `${Math.min(100, ((profile?.xp ?? 410) % 100) || 64)}%` }} />
          </div>
        </div>

        <div className="mt-5 grid grid-cols-3 gap-3">
          {[["Workouts", stats.workouts], ["Streak", stats.streak], ["Records", stats.records]].map(([label, value]) => (
            <div key={label} className="rounded-[22px] border border-black/5 bg-[#faf7f3] p-4 text-center">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          {achievements.map((achievement) => (
            <button
              ...
              className={`rounded-[22px] border p-4 text-left ${achievement.unlocked ? "border-gold/20 bg-[#fbf3df]" : "border-black/5 bg-[#faf7f3]"}`}
            >
      ...
      <section className="rounded-[24px] border border-black/5 bg-card p-5 shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
        ...
        <div className="mt-4 grid grid-cols-3 gap-3">
          {[["Height", "170 cm"], ["Weight", "49.8 kg"], ["BMI", "17.2"]].map(([label, value]) => (
            <div key={label} className="rounded-2xl border border-black/5 bg-[#faf7f3] p-4">
              <p className="text-xs uppercase tracking-[0.24em] text-text3">{label}</p>
      ...
      <section className="rounded-[24px] border border-black/5 bg-card p-5 shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
        ...
        <div className="mt-4 space-y-3">
          {routines.map((routine) => (
            <button key={routine.id} type="button" onClick={() => onOpenRoutine(routine)} className="flex w-full items-center justify-between rounded-2xl border border-black/5 bg-[#faf7f3] p-4 text-left">
              <div>
      ...
      <section className="rounded-[24px] border border-[#efcfbf] bg-gradient-to-br from-[#fff4ed] to-card p-5 shadow-[0_14px_28px_rgba(110,94,74,0.08)]">
