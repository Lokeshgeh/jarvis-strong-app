    ...
    <div className="space-y-5">
      <section className="rounded-[24px] border border-white/10 bg-card p-5">
        ...
        <input
          ...
          className="mt-3 w-full rounded-2xl border border-white/10 bg-[#090912] px-4 py-3 text-text outline-none"
        />
      </section>
      ...
      <section className="rounded-[24px] border border-white/10 bg-card p-5">
        ...
        <div className="mt-4 space-y-3">
          {pending.map((profile) => (
            <div key={profile.id} className="flex items-center justify-between rounded-2xl border border-white/8 bg-[#090912] p-4">
              ...
              <button type="button" onClick={() => onAddFriend(profile.id)} className="rounded-full bg-blue px-4 py-2 text-sm font-bold text-[#03131d]">
                Add
              </button>
            </div>
          ))}
        ...
        {filteredFriends.map((friend) => (
          <article key={friend.id} className="rounded-[24px] border border-white/10 bg-card p-5">
            ...
            <div className="mt-4 grid grid-cols-3 gap-3">
              <div className="rounded-2xl border border-white/8 bg-[#090912] p-4">
            ...
            <button type="button" onClick={() => onRemoveFriend(friend.friend_id)} className="mt-4 rounded-full border border-red/30 px-4 py-2 text-sm text-red">
              Remove Friend
            </button>
          </article>
        ))}
