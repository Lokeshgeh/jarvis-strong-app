...
export default function SettingsModal({ open, onClose, profile, stats, onSave, onExport, onReset, onSignOut }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/65 p-4 backdrop-blur-sm">
      <div className="glass-card w-full max-w-[480px] rounded-[24px] border border-white/10 bg-card p-5 shadow-[0_30px_60px_rgba(2,6,23,0.55)]">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button type="button" onClick={onClose} className="rounded-full border border-white/10 bg-[#0f172a] px-3 py-2 text-sm text-text2">
            Close
          </button>
        </div>
        ...
        <div className="space-y-4">
          <label className="block">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-white/10 bg-[#0f172a] px-4 py-3 text-text outline-none"
            />
          </label>
          ...
          <label className="block">
            ...
            <textarea
              ...
              className="min-h-[90px] w-full rounded-2xl border border-white/10 bg-[#0f172a] px-4 py-3 text-text outline-none"
            />
          </label>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <button type="button" onClick={onExport} className="rounded-2xl border border-white/10 bg-[#0f172a] px-4 py-3 text-sm text-text2">
            Export JSON
          </button>
          <button type="button" onClick={onReset} className="rounded-2xl border border-red/30 bg-red/10 px-4 py-3 text-sm text-red">
            Reset all
          </button>
          <button type="button" onClick={onSignOut} className="rounded-2xl border border-white/10 bg-[#0f172a] px-4 py-3 text-sm text-text2">
            Sign out
          </button>
        </div>

        <div className="mt-5 flex gap-3">
          <button type="button" onClick={onClose} className="flex-1 rounded-2xl border border-white/10 bg-[#0f172a] px-4 py-3 text-text2">
            Cancel
          </button>
          <button
            type="button"
